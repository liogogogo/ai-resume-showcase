...
- `代码状态`: 主开发仓库（private）持续更新，本仓库按阶段同步关键能力
- `最近整理`: 2026-03（补充审计引擎实现、LangGraph 取舍、稳定性策略）
- `可对外信息`: 系统架构、方法论、工程化实践、能力边界
- `不公开内容`: 私有 prompt 细节、生产配置、业务数据
...
- 增补诚信审计引擎的判定链路和阈值策略（相似度区间 + 打回机制）
- 补充了 LangGraph 相对线性 chain 的决策依据（状态管理、分支、迭代）
- 明确了 LLM 输出稳定性方案（结构化输出、重试、rubric、分段处理）
- 补充 Token 成本治理策略（分模型调用、缓存、轮数上限）
